...
export async function createProductsService(data: ProductCreate) {
    ...
    const productsCategoriesRepository = AppDataSource.getRepository(CategoryFood)

    const validCategory = productsCategoriesRepository.findAndCountBy({ id: data.categoryFoodId })
